...
router.group(()=>{
    router.group(()=>{
        ...
        router.get('/user/get',[subcribersController,'getUserSuscription']).where('id',router.matchers.uuid())
        router.get('/event/:id/check',[subcribersController,'HasSubscribe']).where('id',router.matchers.uuid())
